...
// 历史值 recommend/discstats/card/table 已由 migrateViewState 一次性迁移；2026-08 IA 重构起
// simulate→plan（「提升规划」），旧值同样经 migrateViewState 一次迁移后不再兼容。
export const VIEWS = {
  MY_CHARS: 'mychars',
  DISC: 'discs',
  SIMULATE: 'simulate',
 ...
 *  持久化的值是 wiki/discs/…，用值当键查恒为 undefined，四个视图会全部被判非法回退 mychars。 */
export const VIEW_VALUES = new Set(Object.values(VIEWS));
/** 旧 view 持久化值 → 当前值（一次性迁移映射）。⚠️ 旧「统计」(stats) 在 URL 里要按 tab 分流到三个不同去处，
 *  那部分逻辑在 web/urlState.js 的 migrateLegacyStats；本表用于 user-config 里「无 tab 信息」的归一（stats 只能落 mychars）。
 *  迁移批次：2026-11 recommend/discstats/card/table；2026-08 IA 重构 simulate→plan、统计页退役。 */
export const LEGACY_VIEW_MAP = Object.freeze({
  ...
  table: 'mychars',
  stats: 'mychars',
  // 2026-08 练度规划（goal）退役：并入我的角色·汇总表手风琴；goal/plan 旧值不迁移，非法回退 mychars。
